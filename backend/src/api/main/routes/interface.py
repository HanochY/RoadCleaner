...
async def delete_interface(id: UUID, current_user: Annotated[UserPrivate,
                                                    Security(authorize_user, 
                                                    scopes=["interface"])]) -> None:
    response: InterfacePublic = await controller.delete(current_user=current_user, id=id)
    return response

@router.patch('/{id}/undelete', status_code=status.HTTP_200_OK)
async def undelete_interface(id: UUID, current_user: Annotated[UserPrivate,
                                                    Security(authorize_user, 
                                                    scopes=["interface"])]) -> InterfacePublic:
    response: InterfacePublic = await controller.undelete(id=id)
    return response

@router.delete('/{id}', status_code=status.HTTP_204_NO_CONTENT)
async def hard_delete_interface(id: UUID, 
                           current_user: Annotated[UserPrivate,
                                                    Security(authorize_user, 
                                                    scopes=["interface"])]) -> None:
