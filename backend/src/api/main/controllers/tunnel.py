...
class TunnelController(Controller[TunnelModel, TunnelFullInput, TunnelPartialInput, TunnelPublic]):
    ...
    async def create(self, current_user: UserPrivate, data: TunnelFullInput) -> UUID:
        try:
            async for session in generate_db_session():
                object: TunnelModel = await self.repository.create(session=session, author_id=current_user.id, **(data.model_dump()))
    ...
    async def read_by_id(self, current_user: UserPrivate, id: UUID) -> TunnelPublic:
        async for session in generate_db_session():
    ...
    async def update(self, current_user: UserPrivate, id: UUID, new_data: TunnelPartialInput) -> None:
        try:
            async for session in generate_db_session():
    ...
    async def partial_update(self, current_user: UserPrivate, id: UUID, new_data: TunnelPartialInput) -> TunnelPublic:
        try:
            async for session in generate_db_session():
    ...
    async def undelete(self, current_user: UserPrivate, id: UUID) -> TunnelPublic:
        try:
            async for session in generate_db_session():
